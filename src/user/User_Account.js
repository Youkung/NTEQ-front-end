...
function Account() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://nteq-back-end.vercel.app/api/user/profile/update', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      <div className="d-flex flex-grow-1">
          ...
          <div className="p-4">
            <div className="bg-white p-4 rounded shadow-sm">
              <Row>
                ...
                <Col md={8}>
                  <h2 className="mb-4" style={{ fontSize: '2rem', color: '#333' }}>โปรไฟล์</h2>
        ...
        <Modal show={showModal} onHide={() => setShowModal(false)} centered size="lg" >
          <Modal.Header closeButton>
            <Modal.Title>เปลี่ยนรูปโปรไฟล์</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <Form>
              <Form.Group>
                <Form.Label>อัพโหลดรูปภาพใหม่</Form.Label>
                <Form.Control
                  type="file"
